...
int lunar_extended_test(void) {
  ...
  failed += test_result("(Lunar) Arg Latitude J2000", res, ln_deg_to_rad(93.27),
                        10.0);

  /* Long Asc Node */
  res = ln_get_lunar_long_asc_node(JD);
  /* Mean long asc node at J2000 is 125.04 */
  failed += test_result("(Lunar) Long Asc Node J2000", res,
                        ln_deg_to_rad(125.04), 1.0);

  /* Long Perigee */
  res = ln_get_lunar_long_perigee(JD);
  /* Mean long perigee at J2000 is 83.35 */
  failed +=
      test_result("(Lunar) Long Perigee J2000", res, ln_deg_to_rad(83.35), 1.0);
