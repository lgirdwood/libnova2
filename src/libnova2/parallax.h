 ...
 *
 * - **Equatorial Horizontal Parallax**: The angle subtended by the Earth's
 * equatorial radius as seen from the object.
 *   - \f$ \sin(\pi) = \frac{R_{\oplus}}{\Delta} \f$
 *   - \f$ R_{\oplus} \f$: Earth's equatorial radius (6378.14 km).
 *   - \f$ \Delta \f$: Distance to the object.
 *
 * - **Topocentric Coordinates**: Position seen from a specific location on
 * Earth's surface.
 *
 ...
 *
 * Calculates the shift in Right Ascension and Declination due to the observer's
 * location on Earth.
 */
void LIBNOVA2_EXPORT ln2_get_parallax(struct ln_equ_posn *object,
                                      double au_distance,
 ...
 */
void LIBNOVA2_EXPORT ln2_get_parallax_ha(struct ln_equ_posn *object,
                                         double au_distance,
                                         struct ln_lnlat_posn *observer,
