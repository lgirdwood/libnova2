...
int solar_coord_test(void)
{
	...
	failed += test_result("(Solar Coords) L on JD 2448908.5", ln_rad_to_deg(pos.L), 199.907372, 0.00001);
	failed += test_result("(Solar Coords) B on JD 2448908.5", ln_rad_to_deg(pos.B), 0.00000000, 0.00001);
	failed += test_result("(Solar Coords) R on JD 2448908.5", pos.R, 0.99760775, 0.00001);
