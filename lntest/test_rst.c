...
int ell_rst_test(void)
{
	...
	orbit = LN_ELL_ORBIT_DEG(1014.2022026431, 0.9997730, 124.92379, 130.17654, 188.04546, 0.0,
							 ln_get_julian_day(&date) + 0.39481);
...
int hyp_future_rst_test(void)
{
	...
	circumpolar = ln_get_hyp_body_next_rst_horizon(JD, &observer, &orbit, 0, &rst);
...
int body_future_rst_test(void)
{
	...
	circumpolar = ln_get_body_next_rst_horizon_future(JD, &observer, ln_get_solar_equ_coords, LN_SOLAR_STANDART_HORIZON,
													  300, &rst);
