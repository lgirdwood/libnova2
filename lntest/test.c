...
static void start_timer(void) { gettimeofday(&start, NULL); }
...
static int test_result(char *test, double calc, double expect,
                       double tolerance) {
  double diff;
...
static int julian_test(void) {
  ...
  failed +=
      test_result("(Julian Day) Diferrence between two successive "
...
static int nutation_test(void) {
  ...
  failed += test_result("(Nutation) ecliptic (deg) for JD 2446895.5",
                        nutation.ecliptic, 23 + 26 / 60.0 + 27.407 / 3600.0,
                        0.000001);
...
static int aberration_test(void) {
  ...
  failed += test_result("(Aberration) RA  ", ln_rad_to_deg(pos.ra), 41.06238352,
                        0.00000001);
...
static int precession_test(void) {
  ...
  failed +=
      test_result("(Precession 2) RA on JD 2451545.0  ", ln_rad_to_deg(pos2.ra),
                  ln_rad_to_deg(object.ra), 0.00001);
  failed +=
      test_result("(Precession 2) DEC on JD 2451545.0  ",
  ...
  failed += test_result("(Precession 2) RA on B1900  ", ln_rad_to_deg(pos2.ra),
                        20.69538, 0.002);
  failed += test_result("(Precession 2) DEC on B1900  ",
                        ln_rad_to_deg(pos2.dec), 88.7739388888, 0.0001);

  ln_get_equ_pm(&object, &pm, JD2050, &pos);

  ln_get_equ_prec2(&pos, JD2000, JD2050, &pos2);

  failed += test_result("(Precession 2) RA on J2050  ", ln_rad_to_deg(pos2.ra),
                        57.0684583320, 0.003);
...
int lunar_test() {
  ...
  fprintf(stdout, "lunar RA %f  Dec %f\n", ln_rad_to_deg(equ.ra),
          ln_rad_to_deg(equ.dec));
...
int elliptic_motion_test() {
  ...
  failed += test_result("(Equation of kepler) E when e is 0.1 and M is 5.0   ",
                        E, 5.554589253872320, 0.000000000001);
  ...
  failed += test_result("(Dec) for comet Enckle   ",
                        ln_rad_to_deg(equ_posn.dec), 19.13924815, 0.00000001);
...
int parabolic_motion_test() {
  ...
  failed += test_result("(RA) for comet C/2002 X5 (Kudo-Fujikawa)   ",
                        ln_rad_to_deg(equ_posn.ra), 287.79617309, 0.00000001);
  failed += test_result("(Dec) for comet C/2002 X5 (Kudo-Fujikawa)   ",
                        ln_rad_to_deg(equ_posn.dec), 14.11800859, 0.00000001);
...
int hyperbolic_motion_test() {
  ...
  failed += test_result("(Dec) for comet C/2001 Q4 (NEAT)   ",
                        ln_rad_to_deg(equ_posn.dec), 58.6116666666, 0.001);
...
int rst_test() {
  ...
  if (!ret) {
    ...
    failed += test_result("Arcturus rise hour on 2006/01/17 at (15 E,51 N)",
                          date.hours, 21, 0);
    failed += test_result("Arcturus rise minute on 2006/01/17 at (15 E,51 N)",
                          date.minutes, 40, 0);

    ln_get_date(rst.transit, &date);
    failed += test_result("Arcturus transit hour on 2006/01/17 at (15 E,51 N)",
                          date.hours, 5, 0);
    failed +=
        test_result("Arcturus transit minute on 2006/01/17 at (15 E,51 N)",
                    date.minutes, 29, 0);

    ln_get_date(rst.set, &date);
    failed += test_result("Arcturus set hour on 2006/01/17 at (15 E,51 N)",
                          date.hours, 13, 0);
    failed += test_result("Arcturus set minute on 2006/01/17 at (15 E,51 N)",
                          date.minutes, 14, 0);
  }
  ...
  if (!ret) {
    failed += test_result("Arcturus next date is less then transit time",
                          (JD_next < rst.transit), 1, 0);
    failed += test_result("Arcturus next transit time is less then set time",
                          (rst.transit < rst.set), 1, 0);
    failed += test_result("Arcturus next set time is less then rise time",
                          (rst.set < rst.rise), 1, 0);

    ln_get_date(rst.rise, &date);
    failed +=
        test_result("Arcturus next rise hour on 2006/01/17 at (15 E,51 N)",
                    date.hours, 21, 0);
    failed +=
        test_result("Arcturus next rise minute on 2006/01/17 at (15 E,51 N)",
                    date.minutes, 40, 0);

    ln_get_date(rst.transit, &date);
    failed +=
        test_result("Arcturus next transit hour on 2006/01/17 at (15 E,51 N)",
                    date.hours, 5, 0);
    failed +=
        test_result("Arcturus next transit minute on 2006/01/17 at (15 E,51 N)",
                    date.minutes, 29, 0);

    ln_get_date(rst.set, &date);
    failed += test_result("Arcturus next set hour on 2006/01/17 at (15 E,51 N)",
                          date.hours, 13, 0);
    failed +=
        test_result("Arcturus next set minute on 2006/01/17 at (15 E,51 N)",
                    date.minutes, 14, 0);
  }
  ...
  if (!ret) {
    ...
    failed += test_result("Arcturus next set time is less then rise time",
                          (rst.set < rst.rise), 1, 0);
    failed += test_result("Arcturus next rise time is less then transit time",
    ...
    ln_get_date(rst.transit, &date);
    failed +=
        test_result("Arcturus next transit hour on 2006/01/18 at (15 E,51 N)",
                    date.hours, 5, 0);
    failed +=
        test_result("Arcturus next transit minute on 2006/01/18 at (15 E,51 N)",
                    date.minutes, 25, 0);
  ...
  if (!ret) {
    ...
    ln_get_date(rst.set, &date);
    failed += test_result("Arcturus next set hour on 2006/01/18 at (15 E,51 N)",
                          date.hours, 13, 0);
    failed +=
        test_result("Arcturus next set minute on 2006/01/18 at (15 E,51 N)",
                    date.minutes, 10, 0);
  }
  ...
  if (!ret) {
    ...
    ln_get_date(rst.rise, &date);
    failed +=
        test_result("Arcturus next rise hour on 2006/01/18 at (15 E,51 N)",
                    date.hours, 21, 0);
    failed +=
        test_result("Arcturus next rise minute on 2006/01/18 at (15 E,51 N)",
                    date.minutes, 37, 0);
  ...
  failed += test_result("Arcturus sometimes rise above 20 deg at 15 E, 51 N",
                        ret, 0, 0);
  ...
  if (!ret) {
    ln_get_date(rst.rise, &date);
    failed += test_result("Arcturus rise hour on 2006/01/17 at (15 E,51 S)",
                          date.hours, 1, 0);
    failed += test_result("Arcturus rise minute on 2006/01/17 at (15 E,51 S)",
                          date.minutes, 7, 0);

    ln_get_date(rst.transit, &date);
    failed += test_result("Arcturus transit hour on 2006/01/17 at (15 E,51 S)",
                          date.hours, 5, 0);
    failed +=
        test_result("Arcturus transit minute on 2006/01/17 at (15 E,51 S)",
                    date.minutes, 29, 0);

    ln_get_date(rst.set, &date);
    failed += test_result("Arcturus set hour on 2006/01/17 at (15 E,51 S)",
                          date.hours, 9, 0);
    failed += test_result("Arcturus set minute on 2006/01/17 at (15 E,51 S)",
                          date.minutes, 51, 0);
  }

  ret = ln_get_object_rst_horizon(JD, &observer, &object, -20, &rst);
  failed += test_result("Arcturus sometimes rise above -20 deg at 15 E, 51 S",
                        ret, 0, 0);
  ...
  failed +=
      test_result("Sun sometimes rise on 2006/01/17 at 15 E, 51 S", ret, 0, 0);

  if (!ret) {
    ln_get_date(rst.rise, &date);
    failed += test_result("Sun rise hour on 2006/01/17 at (15 E,51 S)",
                          date.hours, 3, 0);
    failed += test_result("Sun rise minute on 2006/01/17 at (15 E,51 S)",
                          date.minutes, 11, 0);

    ln_get_date(rst.transit, &date);
    failed += test_result("Sun transit hour on 2006/01/17 at (15 E,51 S)",
                          date.hours, 11, 0);
    failed += test_result("Sun transit minute on 2006/01/17 at (15 E,51 S)",
                          date.minutes, 9, 0);

    ln_get_date(rst.set, &date);
    failed += test_result("Sun set hour on 2006/01/17 at (15 E,51 S)",
                          date.hours, 19, 0);
    failed += test_result("Sun set minute on 2006/01/17 at (15 E,51 S)",
                          date.minutes, 7, 0);
  }
...
static int ell_rst_test(void) {
  ...
  ln_get_ell_body_equ_coords(JD, &orbit, &pos);
  failed += test_result("(RA) for Hyakutake 1996/03/23 17:38", pos.ra, 221.2153,
                        0.001);
  ...
  if (!ret) {
    ...
    failed += test_result("Hyakutake transit hour on 1996/03/23 at 135 E, 35 N",
                          date.hours, 17, 0);
    failed +=
        test_result("Hyakutake transit minute on 1996/03/23 at 135 E, 35 N",
                    date.minutes, 27, 0);
  ...
  if (!ret) {
    ln_get_date(rst.rise, &date);
    failed +=
        test_result("Hyakutake next rise hour on 1996/03/23 at 135 E, 35 N",
                    date.hours, 9, 0);
    failed +=
        test_result("Hyakutake next rise minute on 1996/03/23 at 135 E, 35 N",
                    date.minutes, 31, 0);

    ln_get_date(rst.transit, &date);
    failed +=
        test_result("Hyakutake next transit hour on 1996/03/24 at 135 E, 35 N",
                    date.hours, 17, 0);
    failed += test_result(
    ...
    ln_get_date(rst.set, &date);
    failed +=
        test_result("Hyakutake next set hour on 1996/03/23 at 135 E, 35 N",
                    date.hours, 1, 0);
    failed +=
        test_result("Hyakutake next set minute on 1996/03/23 at 135 E, 35 N",
                    date.minutes, 49, 0);
  }
...
static int angular_test(void) {
  ...
  failed += test_result("(Angular) Separation of Arcturus and Spica   ", d,
                        32.79302684, 0.00001);

  d = ln_get_rel_posn_angle(&posn1, &posn2);
  failed += test_result("(Angular) Position Angle of Arcturus and Spica   ", d,
                        22.39042787, 0.00001);
...
static int airmass_test(void) {
  ...
  failed += test_result("(Airmass) Altitude at airmass 1", ln_rad_to_deg(X),
                        90.0, 0.01);

  for (x = -10; x < 90; x += 10.54546456) {
    res =
        ln_get_alt_from_airmass(ln_get_airmass(ln_deg_to_rad(x), 750.0), 750.0);
    failed += test_result("(Airmass) Altitude->Airmass->Altitude",
                          ln_rad_to_deg(res), x, 0.0001);
  }
...
static int elliptic_parabolic_test(void) {
  ...
  ln_get_ell_body_next_rst_horizon_future(JD, &observer, &orbit, -10.0, 10,
                                          &rst);
  ...
  /* ln_solve_barker(q, t) */
  res = ln_solve_barker(1.0,
                        100.0); /* this takes only t? No, one arg usually t?
  ...
  ln_get_par_body_next_rst_horizon_future(JD, &observer, &porbit, -10.0, 10,
                                          &rst);
  ...
  */
  ln_get_hyp_body_next_rst_horizon_future(JD, &observer, &horbit, -10.0, 10,
                                          &rst);
...
static int planetary_rect_rst_test(void) {
  ...
  if (rect.X == 0 && rect.Y == 0 && rect.Z == 0)
    failed++;
  ln_get_mercury_rst(JD, &observer, &rst);
  if (rst.rise == 0 &&
      rst.set == 0) { /* might be valid but unlikely for mercury */
  }
  ...
  /* Note: older libnova might not have pluto rst? unchecked but listed in
   * header */
  ...
  /* ln_get_pluto_rst is NOT in missing_functions.txt. It implies it might be
   * tested or not public? */
...
static int utility_conversion_test(void) {
  ...
  failed += test_result("(Utility) hlnlat_to_lnlat lng",
                        ln_rad_to_deg(lnlat.lng), 100.0, 1e-6);
  failed += test_result("(Utility) hlnlat_to_lnlat lat",
                        ln_rad_to_deg(lnlat.lat), 50.0, 1e-6);
  ...
  /* Interpolation */
  res = ln_interpolate3(0.5, 0.0, 1.0,
                        2.0); /* y2 is point at n=0. values are at -1, 0, 1 */
