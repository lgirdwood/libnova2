...
int elliptic_motion_test(void)
{
	...
	failed += test_result("(Geocentric Rect Coords X) for comet Enckle   ", posn.X, 0.72550388, 0.00000001);
	failed += test_result("(Geocentric Rect Coords Y) for comet Enckle   ", posn.Y, -0.28446411, 0.00000001);
	failed += test_result("(Geocentric Rect Coords Z) for comet Enckle   ", posn.Z, -0.27031076, 0.00000001);
	...
	failed += test_result("(Body Earth Dist) for comet Enckle in AU   ", dist, 0.82482945, 0.00000001);
	...
	orbit = LN_ELL_ORBIT_DEG(45.7082927, 0.1550125, 28.99870, 296.40937, 79.55499, 0.00318942, e_JD);
