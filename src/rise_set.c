...
int ln_get_body_next_rst_horizon(
    ...
    void (*get_equ_body_coords)(double, struct ln_equ_posn *), double horizon,
    struct ln_rst_time *rst) {
  return ln_get_body_next_rst_horizon_future(JD, observer, get_equ_body_coords,
                                             horizon, 1, rst);
}
...
int ln_get_body_next_rst_horizon_future(
  ...
  if (!ret && (rst->rise > (JD + 0.5) || rst->transit > (JD + 0.5) ||
    ...
    ret = ln_get_body_rst_horizon_offset(JD - 1, observer, get_equ_body_coords,
                                         horizon, &rst_1, nan("0"));
    if (ret)
  ...
  if (ret || (rst->rise < JD || rst->transit < JD || rst->set < JD)) {
    ...
    while (day <= day_limit) {
      ret = ln_get_body_rst_horizon_offset(
          JD + day, observer, get_equ_body_coords, horizon, &rst_2, nan("0"));
